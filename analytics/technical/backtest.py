...
class Backtest(Strategy, Wallet):
    ...
    def run(self):
        ...
        print("Setting up...")
        self._load_data()
        self.update_indicators(self.data_)
    ...
    def _create_periods(self):
        ...
        SETUP_LIST = [{"delta_in_months": 3, "duration_in_years": 3},
                      {"delta_in_months": 1, "duration_in_years": 10}]

        # Loop setups
        self._periods = []
        minimum_date = min(self.data_.index)
        maximum_date = max(self.data_.index)
        for setup in SETUP_LIST:
            delta = setup["delta_in_months"]
            duration = setup["duration_in_years"]

            # Loop periods
            current_date = minimum_date
            for _ in range(MAXIMUM_ITERATIONS):
                final_date = current_date + relativedelta(years=duration)
                final_date = \
                    final_date if final_date < maximum_date else maximum_date
                new_period = {"from": current_date, "to": final_date}
                self._periods.append(new_period)
                current_date = current_date + relativedelta(months=delta)
    ...
    def _evaluate(self, period):
        ...
        data = self.data_
        data = \
            data[(data.index > period["from"]) & (data.index < period["to"])]
        ...
        for index in sorted(set(data.index)):

            # Update wallet
            index_data = data[data.index == index]
            index_data = index_data.set_index("ticker")
            past_deals = self.update_wallet(past_deals, index_data, index)

            # Update deals
            index_indicators = indicators[indicators.index == index]
            new_deals = self.get_deals(index_indicators)
            past_deals.update(new_deals)
    ...
    def _evaluate_baseline(self, period):
        ...
        # Evaluate
        initial_patrimony = data["close"].iloc[0]
        final_patrimony = data["close"].iloc[-1]
        profit = final_patrimony - initial_patrimony
        profit_percentage = profit / initial_patrimony

        # Results
        results = data["close"].to_frame("patrimony")
    ...
    def _load_data(self):
        ...
        MIN_MEAN_VOLUME = 500000
        N_PAST_SAMPLES = 60  # Roughly 3 months
        BASELINE_TICKER = "IBOV"
        BLOCKED_TICKERS = ["GFSA3"]
        ...
        # Filter data
        data = data[(data.index > INITIAL_DATE) & (data.index < FINAL_DATE)]
        ...
        for ticker in set(data["ticker"]):
            ...
            last_samples = ticker_data.iloc[-N_PAST_SAMPLES:]
            case_1 = last_samples["volume"].mean() < MIN_MEAN_VOLUME
            case_2 = ticker in BLOCKED_TICKERS
            if case_1 or case_2:
