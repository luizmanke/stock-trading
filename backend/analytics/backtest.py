...
class Backtest(Strategy, Wallet):
    ...
    def run(self):
        ...
        for i, period in enumerate(self._periods):
            ...
            # Compare
            better_months = self._get_better_months(monthly_stats, base_monthly_stats)
    ...
    def _compute_indicators(self):
        ...
        # Get variables
        minimum_index = min([period["from"] for period in self._periods])
        ...
        # Loop indexes
        indicators = []
        indexes = sorted(set(fundamentals.index))
        indexes = [index for index in indexes
                   if (index >= minimum_index and index <= maximum_index)]
